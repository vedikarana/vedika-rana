# Hi, I'm Vedika Rana 

B.Tech CSE (AI & Data Science) — MIT-WPU, Pune   
CFA Level 1 Candidate · Portfolio Manager · Wealth Advisor  
Building at the intersection of **AI + Finance + Product**  
Founder, Roots & RoastCO  
Pune, Maharashtra, India

---

## Who I Am

I'm not just a CS student — I've managed **150+ client portfolios** at Rana Wealth,
boosted returns by **23% YoY**, founded a consumer brand, and built AI systems
that run on satellite data, sentiment analysis, and explainable ML.

I bring **real finance depth + technical execution** to every project I touch.

---

## Experience

**Portfolio Manager — Rana Wealth** *(Jun 2024 – Present)*  
- Managed multi-asset portfolios for 150+ clients (aged 18–50)
- Data-driven asset allocation → **+23% YoY returns**
- Identified diversification trends across equity, debt, and alternative assets

**Founder — Roots & RoastCO** *(Jun 2025 – Present)*  
- Founded a premium matcha + specialty coffee brand targeting Gen Z
- Sourced Kyoto matcha; scaled through 10+ specialized workshops
- Built brand, ops, and community from zero

---

## 🛠 Tech Stack

**AI / ML**  
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=flat&logo=tensorflow&logoColor=white)
![Scikit-Learn](https://img.shields.io/badge/Scikit--Learn-F7931E?style=flat&logo=scikit-learn&logoColor=white)
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white)

**Web / Mobile**  
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat&logo=nextdotjs&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=flat&logo=typescript&logoColor=white)
![Flutter](https://img.shields.io/badge/Flutter-02569B?style=flat&logo=flutter&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=flat&logo=firebase&logoColor=black)

**Data & Viz**  
![Tableau](https://img.shields.io/badge/Tableau-E97627?style=flat&logo=tableau&logoColor=white)
![Power BI](https://img.shields.io/badge/PowerBI-F2C811?style=flat&logo=powerbi&logoColor=black)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=mysql&logoColor=white)

---

##  Finance Credentials
```
CFA Institute    →  CFA Level 1 Candidate
Rana Wealth      →  Portfolio Manager · 150+ clients · +23% YoY
Focus Areas      →  Equity, Fixed Income, Derivatives, Risk,
                    Algorithmic Trading, XAI in BFSI
```


## Publications & Patents

- 📋 **Patent (filing):** Explainable AI for Compliance and Personalized Investment
- 📄 **Research Paper:** Stock Market Prediction Using Machine Learning


## Open To

- **FinTech / Quant Analyst** — where finance + ML both matter
- **AI in BFSI** — explainable models for banking, investment, insurance
- **Product / Full-Stack roles** — AI-powered consumer or enterprise products
- **Wealth Tech** — robo-advisory, portfolio analytics, digital investment platforms

---

## Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/vedika-rana)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)](https://github.com/vedikarana)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:vedikarana14@gmail.com)
 
